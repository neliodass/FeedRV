        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-blue-500/50 transition-all group">
            ...
            <div className="p-4">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    {item.tags.map(tag => (
                        <span key={tag.name} className="px-2 py-1 bg-slate-800 text-slate-300 text-xs rounded-md">#{tag.name}</span>
                    ))}
                </div>
                <a href={item.url} target="_blank" className="flex items-center gap-2 text-sm text-blue-400 hover:text-blue-300 font-medium">
